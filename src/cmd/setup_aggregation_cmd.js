...
const { getAggregationSetup } = require('../setup/generateAggregationSetup.js');

module.exports = async function setupAggregationCmd(proofManagerConfig, buildDir = "tmp") {
    const setupOptions = {
    ...
    const globalInfoPath = path.join(proofManagerConfig.provingKeyPath, "pilout.globalInfo.json");
    ...
    const vadcopFinalDir = path.join(proofManagerConfig.provingKeyPath, globalInfo.name, "vadcop_final");
    ...
    log.info("[Setup Aggregation Cmd]", `Reading vadcop_final setup files from ${vadcopFinalDir}`);
    ...
    log.info("[Setup Aggregation Cmd]", `Generating final compressed setup for '${globalInfo.name}'`);
    await getAggregationSetup(
        buildDir, setupOptions, constRootFinal,
        starkInfoFinal, verifierInfoFinal, proofManagerConfig.publicsInfo
    );

    log.info("[Setup Aggregation Cmd]", "Final compressed setup completed successfully");
}
