...
module.exports.addInfoExpressions = function addInfoExpressions(expressions, exp, evMap) {
    ...
    if (exp.op == "exp") {
        addInfoExpressions(expressions, expressions[exp.id], evMap);
        ...
        if(["cm", "custom", "const"].includes(exp.op)) {
            let ev = {type: exp.op, id: exp.id, prime: exp.rowOffset};
            if(exp.op === "custom") ev.commitId = exp.commitId;
            if(evMap && !evMap.find(e => e.type === ev.type && e.id === ev.id && e.prime === ev.prime)) {
                evMap.push(ev);
            }
        }
        ...
        addInfoExpressions(expressions, lhsValue, evMap);
        addInfoExpressions(expressions, rhsValue, evMap);
